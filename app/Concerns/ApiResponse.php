...
trait ApiResponse
{
    ...
    {
        return $this->responseMessage ?? __('messages.respond.successful_message');
    }
    ...
    protected function respondWithError(
        ...
        return $this->setHasErrors(true)
                    ->setMessage($message ?? __('messages.respond.error'))
                    ->respond($constraints, $status);
    }

    protected function respondEntityRemoved(int $id): JsonResponse
    {
        return $this->setMessage(__('messages.respond.entity_removed'))->respond(['id' => $id]);
    }

    protected function respondEntityNotFound(?string $message = null): JsonResponse
    {
        return $this->respondWithError(
            $message ?? __('messages.respond.not_found'),
    ...
    {
        return $this->respondWithError(
            $message ?? __('messages.respond.wrong_parameter'),
    ...
    {
        return $this->respondWithError(
            $message ?? __('messages.respond.invalid_query'),
            status: Response::HTTP_BAD_REQUEST
        );
    }
    ...
    {
        return $this->respondWithError(
            $message ?? __('messages.respond.invalid_parameters'),
    ...
    {
        return $this->respondWithError(
            $message ?? __('messages.respond.unauthorized'),
    ...
    {
        return $this->respondWithError(
            $message ?? __('messages.respond.forbidden'),
    ...
    {
        return $this->respondWithError(
            $message ?? __('messages.respond.not_acceptable'),
